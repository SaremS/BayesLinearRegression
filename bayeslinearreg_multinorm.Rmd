...
logPriors = function(betas,sd, meanNormalPrior = 0, sdNormalPrior = 5, shapeInvGammaPrior = 1, scaleInvGamma = 1)
{
  ...
  #wird ein eizelner Skalar als Standardabweichung angegeben, wird eine Diagonal-(Kovarianz)matrix erstellt,   #bei der alle Elemente der Hauptdiagonalen dem Skalar entsprechen. Zu beachten ist, dass 'mvtnorm' mit den   #(Ko-)Varianzen arbeitet, während ansonsten mit Standardabweichungen gearbeitet wird - also müssen die SDs   #quadriert werden
  if (length(sdNormalPrior) == 1)
  {
    tempCovMatrix = matrix(0, nrow = lenBetas, ncol = lenBetas)
    diag(tempCovMatrix) = rep(sdNormalPrior^2,lenBetas)
    sdNormalPrior = tempCovMatrix
  }
  # alternativ kann die Prior-Standardabweichung für jedes Beta einzeln angegeben werden
  else if(length(sdNormalPrior) == lenBetas)
  {
    tempCovMatrix = matrix(0, nrow = lenBetas, ncol = lenBetas)
    diag(tempCovMatrix) = sdNormalPrior^2
    sdNormalPrior = tempCovMatrix
  }
  
  sumLogBetasPrior = 
    dmvnorm(betas, mean = meanNormalPrior,
          sigma = sdNormalPrior, log = TRUE)
...
#Kovarianzmatrix des OLS-Schätzers für die Betas
covMatPriorExpectation = sqrt(solve(t(priorX)%*%priorX)*sdPriorExpectation^2)
...
bayRegProp1start1 = metropolisBayesRegression(bayesX, bayesY, startValuesBeta = startValBeta1, startValueSd = startValSd1, nIterations = 10000, nBurnIn = 5000, meanNormalPrior = betaPriorMeans, sdNormalPrior = covMatPriorExpectation, shapeInvGammaPrior = alphaPrior, scaleInvGammaPrior = betaPrior, proposalFunction = proposalFunction1, betaProposalScaleParameter = 0.35, sdProposalScaleParameter = 0.35)

print(bayRegProp1start1$acceptanceRate)


bayRegProp1start2 = metropolisBayesRegression(bayesX, bayesY, startValuesBeta = startValBeta2, startValueSd = startValSd2, nIterations = 10000, nBurnIn = 5000, meanNormalPrior = betaPriorMeans, sdNormalPrior = covMatPriorExpectation, shapeInvGammaPrior = alphaPrior, scaleInvGammaPrior = betaPrior, proposalFunction = proposalFunction1, betaProposalScaleParameter = 0.35, sdProposalScaleParameter = 0.35)

print(bayRegProp1start2$acceptanceRate)


bayRegProp1start3 = metropolisBayesRegression(bayesX, bayesY, startValuesBeta = startValBeta3, startValueSd = startValSd3, nIterations = 10000, nBurnIn = 5000, meanNormalPrior = betaPriorMeans, sdNormalPrior = covMatPriorExpectation, shapeInvGammaPrior = alphaPrior, scaleInvGammaPrior = betaPrior, proposalFunction = proposalFunction1, betaProposalScaleParameter = 0.35, sdProposalScaleParameter = 0.35)
...
bayRegProp2start1 = metropolisBayesRegression(bayesX, bayesY, startValuesBeta = startValBeta1, startValueSd = startValSd1, nIterations = 10000, nBurnIn = 5000, meanNormalPrior = betaPriorMeans, sdNormalPrior = covMatPriorExpectation, shapeInvGammaPrior = alphaPrior, scaleInvGammaPrior = betaPrior, proposalFunction = proposalFunction2, betaProposalScaleParameter = 0.35, sdProposalScaleParameter = 0.35)
...
bayRegProp3start1 = metropolisBayesRegression(bayesX, bayesY, startValuesBeta = startValBeta1, startValueSd = startValSd1, nIterations = 10000, nBurnIn = 5000, meanNormalPrior = betaPriorMeans, sdNormalPrior = covMatPriorExpectation, shapeInvGammaPrior = alphaPrior, scaleInvGammaPrior = betaPrior, proposalFunction = proposalFunction3, betaProposalScaleParameter = 0.35, sdProposalScaleParameter = 0.35)
...
```
```{r tableAcceptance, echo = FALSE, results = "asis"}

acceptanceTable = matrix(0, nrow = 3, ncol = 3)
colnames(acceptanceTable) = c("Proposal Function 1", "Proposal Function 2", "Proposal Function 3")
rownames(acceptanceTable) = c("Nahe Anfangsparamter", "Weiter entfernte Betas", "Alle Parameter weiter entfernt" )

acceptanceTable[1,1] = bayRegProp1start1$acceptanceRate
acceptanceTable[1,2] = bayRegProp2start1$acceptanceRate
acceptanceTable[1,3] = bayRegProp3start1$acceptanceRate

acceptanceTable[2,1] = bayRegProp1start2$acceptanceRate
acceptanceTable[2,2] = bayRegProp2start2$acceptanceRate
acceptanceTable[2,3] = bayRegProp3start2$acceptanceRate

acceptanceTable[3,1] = bayRegProp1start3$acceptanceRate
acceptanceTable[3,2] = bayRegProp2start3$acceptanceRate
acceptanceTable[3,3] = bayRegProp3start3$acceptanceRate


library(knitr)
kable(acceptanceTable, caption = "Acceptance Rates")
...
kd2 = ggplot(bayRegProp1start1$chainNoBurnIn, aes(beta1)) + geom_density(color = 2, fill = 2, alpha = 0.1) + geom_vline(xintercept = beta1, color = 4) + ggtitle("KDE beta1") + geom_vline(xintercept = mean(bayRegProp1start1$chainNoBurnIn[,2]), color = 6)

kd3 = ggplot(bayRegProp1start1$chainNoBurnIn, aes(SD)) + geom_density(color = 3, fill = 3, alpha = 0.1) + geom_vline(xintercept = standardDeviation, color = 4) + ggtitle("KDE SD") + geom_vline(xintercept = mean(bayRegProp1start1$chainNoBurnIn[,3]), color = 6)
